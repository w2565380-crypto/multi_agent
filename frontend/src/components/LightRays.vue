...
float rayStrength(vec2 raySource, vec2 rayRefDirection, vec2 coord,
  ...
  float lengthFalloff = clamp((maxDist - distance) / maxDist, 0.0, 1.0);
...
const init = () => {
  ...
  const loop = (t) => {
    ...
    if (props.followMouse && props.mouseInfluence > 0) {
      const s = 0.92
      smoothMouseRef.x = smoothMouseRef.x * s + mouseRef.x * (1 - s)
      smoothMouseRef.y = smoothMouseRef.y * s + mouseRef.y * (1 - s)
...
watch(() => [
  props.raysColor, props.raysSpeed, props.lightSpread,
  props.rayLength, props.pulsating, props.fadeDistance,
  props.saturation, props.mouseInfluence, props.noiseAmount,
  props.distortion, props.raysOrigin,
], updateUniforms)
</script>
