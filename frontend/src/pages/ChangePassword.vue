    ...
    <div class="glass-card">
      <form @submit.prevent="handleSubmit" class="edit-form">
        <div class="form-group">
          <label>原密码</label>
          <input v-model="form.oldPassword" class="apple-input" type="password" placeholder="输入当前密码">
          <span v-if="errors.oldPassword" class="field-error">{{ errors.oldPassword }}</span>
        </div>
        <div class="form-group">
          <label>新密码</label>
          <input v-model="form.newPassword" class="apple-input" type="password" placeholder="至少6位，仅限字母和数字">
          <span v-if="errors.newPassword" class="field-error">{{ errors.newPassword }}</span>
        </div>
        <div class="form-group">
          <label>确认新密码</label>
          <input v-model="form.confirmPassword" class="apple-input" type="password" placeholder="再次输入新密码">
...
const handleSubmit = async () => {
  ...
  if (!form.oldPassword) { errors.oldPassword = '请输入原密码'; return }
  if (!form.newPassword) { errors.newPassword = '请输入新密码'; return }
  if (form.newPassword.length < 6) { errors.newPassword = '密码至少 6 位'; return }
  if (!/^[a-zA-Z0-9]+$/.test(form.newPassword)) { errors.newPassword = '密码仅限字母和数字'; return }
  if (form.newPassword !== form.confirmPassword) { errors.confirmPassword = '两次密码输入不一致'; return }
  ...
  try {
    ...
    const verifyRes = await fetch('/api/users/login', {
      ...
      body: JSON.stringify({ user_name: auth.user.userName, password: form.oldPassword })
    })
    if (!verifyRes.ok) {
      const d = await verifyRes.json().catch(() => ({}))
      throw new Error(d.detail || '原密码错误')
    }
    ...
    if (!res.ok) {
      ...
      throw new Error(d.detail || `修改失败 (${res.status})`)
    }
    router.replace('/profile?msg=密码修改成功')
    return
  } catch (err) {
...
.glass-card { background: rgba(255,255,255,0.72); backdrop-filter: blur(20px); border-radius: 16px; box-shadow: 0 4px 24px rgba(0,0,0,0.08); border: 1px solid rgba(255,255,255,0.9); padding: 24px; }
.edit-form { display: flex; flex-direction: column; gap: 16px; }
.form-group { display: flex; flex-direction: column; gap: 6px; }
.form-group label { font-size: 13px; font-weight: 500; color: #86868B; }

.apple-input { padding: 10px 14px; border-radius: 10px; border: 1px solid rgba(0,0,0,0.1); font-size: 14px; outline: none; font-family: inherit; background: rgba(255,255,255,0.8); transition: border 0.2s; }
