  ...
  <main class="main-wrap">
    ...
    <div class="glass-card chat-card">
      ...
      <div class="bottom-bar">
        ...
        <template v-else-if="project?.status === 'COMPLETED'">
          ...
          <button v-else class="btn btn-preview" @click="showPreview = true">🔗 预览项目</button>
        </template>
    ...
    <div class="modal-overlay" v-if="showPreview" @click.self="showPreview = false">
      <div class="modal-content preview-modal">
        ...
        <div v-else class="chat-empty">正在加载预览...</div>
      </div>
    </div>
  </main>
</template>
...
const inputPlaceholder = computed(() => project.value?.status === 'PENDING_APPROVAL' ? '输入驳回修改意见...' : '输入消息...')
...
const handleReject = async () => {
  ...
  try { await store.approveProject(project.value.id, false, fb); project.value.status = 'RUNNING'; addMsg('系统','系','pm',`已驳回${fb?'：'+fb:''}，产品经理将重新规划。`) } catch(e) { alert(e.message) }
  submitting.value = false
}
...
watch(() => route.params.projectId, async (id) => {
  ...
  loading.value = true; chatMessages.value = []; prdLoaded.value = false; qaLoaded.value = false
  if (store.lastCreateMsg) { addMsg('系统','系','pm',store.lastCreateMsg); store.lastCreateMsg = '' }
  await fetchStatus(); loading.value = false; scrollBottom()
  if (project.value?.status === 'COMPLETED' && !previewUrl.value) loadPreview()
}, { immediate: true })
...
.preview-modal { max-width: none; width: 96vw; height: 94vh; }
.preview-frame { flex: 1; width: 100%; height: 100%; border: none; border-radius: 8px; background: #fff; }
</style>
