...
#     def get_project_dir(self, username: str, project_id: int, project_title: str) -> str:
#         """🌟 核心对齐：拼入 username，与你修改后的 projects_crud.py 保持 100% 一致！"""
#         safe_title = "".join([c for c in project_title if c.isalnum() or c in ('_', '-')])
#         return os.path.join(self.exports_dir, username, f"project_{project_id}_{safe_title}")
...
#     def read_src_codes_combined_by_db_path(self, username: str, project_id: int, project_title: str, db_path: str) -> str:
#         """根据数据库记录的源码路径（如 'src/'），读取并拼接所有代码文件"""
#         project_dir = self.get_project_dir(username, project_id, project_title)
#         src_dir = os.path.join(project_dir, db_path)
#         if not os.path.exists(src_dir):
#             return ""
...
#     def read_file_by_db_path(self, username: str, project_id: int, project_title: str, db_path: str) -> str:
#         """根据数据库里保存的相对路径，安全读取物理文件内容"""
#         project_dir = self.get_project_dir(username, project_id, project_title)
#         full_path = os.path.join(project_dir, db_path)
...
class FileHelper:
    ...
    def get_project_dir(self, user_id: int, project_id: int, project_title: str) -> str:
        """🌟 核心修改：完全采用 user_id 代替可变的用户名进行文件物理存放"""
        safe_title = "".join([c for c in project_title if c.isalnum() or c in ('_', '-')])
        return os.path.join(self.exports_dir, str(user_id), f"project_{project_id}_{safe_title}")

    def save_prd(self, user_id: int, project_id: int, project_title: str, prd_content: str) -> str:
        """物理保存 PRD 到隔离账号目录下"""
        project_dir = self.get_project_dir(user_id, project_id, project_title)
        os.makedirs(project_dir, exist_ok=True)
        prd_path = os.path.join(project_dir, "PRD.md")
        with open(prd_path, 'w', encoding='utf-8') as f:
            f.write(prd_content)
    ...
    def read_prd(self, user_id: int, project_id: int, project_title: str) -> str:
        """读取隔离账号目录下的 PRD"""
        project_dir = self.get_project_dir(user_id, project_id, project_title)
        prd_path = os.path.join(project_dir, "PRD.md")
        if not os.path.exists(prd_path):
    ...
    def save_src_codes(self, user_id: int, project_id: int, project_title: str, files_list: list) -> str:
        """物理保存代码到隔离账号目录下的 src/ 目录"""
        project_dir = self.get_project_dir(user_id, project_id, project_title)
        src_dir = os.path.join(project_dir, "src")
    ...
    def read_src_codes_combined_by_db_path(self, user_id: int, project_id: int, project_title: str, db_path: str) -> str:
        """根据数据库记录的源码路径（如 'src/'），读取并拼接所有代码文件"""
        project_dir = self.get_project_dir(user_id, project_id, project_title)
        src_dir = os.path.join(project_dir, db_path)
        if not os.path.exists(src_dir):
    ...
    def save_qa_report(self, user_id: int, project_id: int, project_title: str, qa_report: str) -> str:
        """物理保存测试报告到隔离账号目录下"""
        project_dir = self.get_project_dir(user_id, project_id, project_title)
        qa_path = os.path.join(project_dir, "Test_Report.md")
        with open(qa_path, 'w', encoding='utf-8') as f:
    ...
    def read_file_by_db_path(self, user_id: int, project_id: int, project_title: str, db_path: str) -> str:
        """根据数据库里保存的相对路径，安全读取物理文件内容"""
        project_dir = self.get_project_dir(user_id, project_id, project_title)
        full_path = os.path.join(project_dir, db_path)
        if not os.path.exists(full_path):
    ...
    def zip_project(self, user_id: int, project_id: int, project_title: str) -> str:
        """打包隔离账号下的项目目录"""
        project_dir = self.get_project_dir(user_id, project_id, project_title)
        user_dir = os.path.dirname(project_dir)
        zip_path = os.path.join(user_dir, f"project_{project_id}.zip")
        ...
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            for root, _, files in os.walk(project_dir):
                for file in files:
                    ...
                    rel_path = os.path.relpath(file_full_path, project_dir)
                    zipf.write(file_full_path, rel_path)
        # 返回对齐 Dify 和打包的相对路径，同样采用 user_id 进行定位
        return f"exports/{user_id}/project_{project_id}.zip"
